document.addEventListener('DOMContentLoaded', () => {
  ...
  computerSquares.forEach(square => square.addEventListener('dragstart', dragStart))
  computerSquares.forEach(square => square.addEventListener('dragover', dragOver))
  computerSquares.forEach(square => square.addEventListener('dragenter', dragEnter))
  computerSquares.forEach(square => square.addEventListener('dragleave', dragLeave))
  computerSquares.forEach(square => square.addEventListener('drop', p2DragDrop))
  computerSquares.forEach(square => square.addEventListener('dragend', dragEnd))
  ...
  function p2DragDrop() {
    ...
    if (isHorizontal && !newNotAllowedHorizontal.includes(shipLastId)) {
      for (let i=0; i < draggedShipLength; i++) {
        computerSquares[parseInt(this.dataset.id) - selectedShipIndex + i].classList.add('taken', shipClass)
      }
    //As long as the index of the ship you are dragging is not in the newNotAllowedVertical array! This means that sometimes if you drag the ship by its
    //index-1 , index-2 and so on, the ship will rebound back to the displayGrid.
    } else if (!isHorizontal && !newNotAllowedVertical.includes(shipLastId)) {
      for (let i=0; i < draggedShipLength; i++) {
        computerSquares[parseInt(this.dataset.id) - selectedShipIndex + width*i].classList.add('taken', shipClass)
      }
    } else return
